"""Port of src/shopify_service.py (P1-3). Signatures preserved.

Changes: settings come from app.settings; request failures are logged before
being reported in the returned status payloads (no silent swallowing).
"""
...
class ShopifyService:
    def __init__(self) -> None:
        ...
        self.store_domain = settings.shopify_store_domain.strip().replace("https://", "").replace("http://", "").strip("/")
        self.token = settings.shopify_admin_access_token.strip()
        self.api_version = settings.shopify_api_version
        self.enabled = bool(self.store_domain and self.token)
        self.write_enabled = bool(settings.shopify_write_enabled)
    ...
    def product_preview(self, limit: int = 20) -> dict:
        ...
        try:
            ...
            logger.warning("Shopify product preview failed: %s", exc)
            status = self.status()
    ...
    def _preview_for_item(self, item: dict, products: list[dict], status: dict) -> dict:
        ...
        return {
            ...
            or (
                ...
                if current_value
                else "Matched product, but this field is currently empty or not returned by Shopify."
            ),
        }
    ...
    def apply_seo_item(self, item: dict) -> dict:
        ...
        if not update:
            return {
                "applied": False,
                "blocked": True,
                "message": f"Unsupported SEO action type: {action_type}",
                "item_id": item.get("id"),
            }
        ...
        try:
            ...
            logger.warning("Shopify SEO apply failed for item %s: %s", item.get("id"), exc)
