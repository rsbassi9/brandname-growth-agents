"""Port of src/local_workflow.py (P1-4): deterministic local-only daily workflow.

No OpenAI text or image calls are made anywhere in this module. The one prose
change vs the legacy module: the brand string comes from settings (P1-7)
...
from .files import save_markdown
...
def run_local_daily_workflow() -> dict[str, str]:
    ...
    paths = {
        "asset_inventory": str(
            save_markdown(
                "asset_inventory",
                f"local-cycle-{cycle}-asset-inventory",
                _asset_inventory_markdown(inventory.summary, buckets, cycle),
            )
        ),
    }
    ...
    paths["content_candidates"] = str(
        save_markdown("content_candidates", f"local-cycle-{cycle}-content-candidates", candidate_markdown)
    )
    ...
    paths["report"] = str(
        save_markdown(
            "daily_reports",
            f"local-cycle-{cycle}-daily-growth-report",
            _report_markdown(paths, candidates, cycle),
        )
    )
    return paths
...
def _bucket_assets(assets: list[dict]) -> dict[str, list[dict]]:
    ...
    for bucket, items in buckets.items():
        ...
        items.sort(
            key=lambda asset: (
                (_last_number(asset.get("name", "")) * (-1 if reverse_numbers else 1)),
...
def _candidate_specs(buckets: dict[str, list[dict]], cycle: int) -> list[dict]:
    ...
    return [
        ...
        },
        {
            ...
            "why": f"Transformation content is the strongest differentiator for {brand} Design.",
            "priority": "High",
        },
        {
...
def _ideas_markdown(candidates: list[dict], cycle: int) -> str:
    lines = [
        f"# Local Content Ideas - Cycle {cycle}",
        "",
        "Approval status: Draft for review",
        "",
        f"Brand vocabulary: {BRAND_WORDS}",
        "",
    ]
    for index, item in enumerate(candidates, start=1):
        lines.extend(
            [
                f"### {index}. {item['title']}",
                f"- Core idea: {item['hook']}",
                f"- Asset basis: {', '.join(item['source_files'])}",
                "",
            ]
        )
    return "\n".join(lines)
...
def _candidates_markdown(candidates: list[dict], cycle: int) -> str:
    lines = [
        f"# Local Content Candidates - Cycle {cycle}",
        "",
        "Approval status: Draft for review",
        "",
    ...
    for item in candidates[:4]:
        lines.extend(
            [
                f"## {item['title']}",
                f"Hook: {item['hook']}",
                f"Caption draft: {item['caption']}",
                f"Asset notes: {', '.join(item['source_files'])}",
                "",
            ]
        )
    return "\n".join(lines)
...
def _seo_markdown(candidates: list[dict], cycle: int) -> str:
    product_names = Counter(
        name.split("-mockups", 1)[0] for item in candidates for name in item["source_files"] if "mockups" in name.lower()
    )
    lines = [
        f"# Local SEO Notes - Cycle {cycle}",
        "",
        "Approval status: Draft for review",
        "",
        "## Product Language",
        "- Use art-translation language before clothing basics.",
        "- Mention cotton/weight only after the concept and object framing.",
        "",
    ]
