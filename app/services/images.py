"""Port of src/image_concepts.py with the P1-2 defect fixes.

Changes vs the legacy module (logic otherwise preserved):
(a) the responses-API model comes from settings (BRAND_IMAGE_MODEL /
    BRAND_TEXT_MODEL), never hardcoded;
(b) the two silent exception blocks (old lines 216-217 and 250-251) now
    logger.exception(...) and raise a typed GenerationError;
(c) all OpenAI calls go through the client wrapper, which raises
    LocalOnlyModeError when LOCAL_ONLY_AGENT_RUNS=true.
"""
...
from ..paths import ROOT_DIR, outputs_dir
from ..settings import get_settings
from .openai_client import GenerationError, get_openai_client, resolve_model
from .rendering import _slug
...
def generate_image_concepts(plan: dict, asset_paths: list[Path]) -> dict[str, str]:
    ...
    if not settings.ai_image_generation_enabled:
        return {}
    concepts = plan.get("image_concepts", [])[: settings.image_concept_count]
    if not concepts:
        return {}
...
def generate_post_visual_image(
    ...
    if not settings.ai_image_generation_enabled:
        raise GenerationError(
            "AI image generation is disabled. Set AI_IMAGE_GENERATION_ENABLED=true to render new AI images."
        )
    ...
    prompt = build_post_prompt(item, concept_type, brief, direction)
    ...
    return {
        ...
        "metadata_path": str(output_dir / "render-metadata.json"),
        "prompt": prompt,
        "concept_type": concept_type,
    }


def _product_inventory_summary() -> str:
    # TODO(fable-review): src.product_inventory is intentionally reused (data
    # helper, no external calls); full port is scheduled with the P5 cutover.
    try:
        from src.product_inventory import product_inventory_summary

        return product_inventory_summary()
    except Exception:
        logger.exception("Could not build product inventory summary; using safe fallback text")
        return (
            "No product image inventory is available. Do not invent garments; "
            "use existing raw product/campaign assets only."
        )


def build_post_prompt(item: dict, concept_type: str, brief: str, direction: str) -> str:
    ...
    return "\n".join(
        [
            f"Create one premium editorial image for {brand} Design.",
            type_instruction,
            ...
            item.get(
                ...
                "Use the selected product folder references to preserve exact garment side, graphic placement, small logo placement, trims, grommets, tags, hems, and silhouette.",
            ),
...
def _build_prompt(concept: dict, plan: dict) -> str:
    ...
    return "\n".join(
        [
            f"Create one premium editorial source image for {brand} Design.",
    ...
    if reference_paths:
        ...
        try:
            response = client.responses.create(
                model=resolve_model(),  # P1-2a: from settings, never hardcoded
            ...
            if image_data:
                return image_data[0]
        except OpenAIError as exc:  # P1-2b: no silent swallowing
            logger.exception("Reference-guided image generation failed")
            raise GenerationError(f"Reference-guided image generation failed: {exc}") from exc
        ...
        try:
            if path.suffix.lower() in {".heic", ".heif"}:
                try:
                    ...
                    # Explicit fallback: HEIC support is optional; skip HEIC refs
                    # when pillow-heif is missing instead of failing the run.
                    logger.warning("pillow-heif not installed; skipping HEIC reference %s", path)
                    continue
            with Image.open(path) as image:
                ...
                prepared.append(out)
        except Exception as exc:  # P1-2b: no silent swallowing
            logger.exception("Failed to prepare reference image %s", path)
            raise GenerationError(f"Failed to prepare reference image {path}: {exc}") from exc
